"""Sweep noise filter thresholds — exit at bar 1-2 if profit_r below threshold."""
...
ft=pd.Timestamp('2026-05-01',tz='UTC');et=pd.Timestamp('2026-05-20',tz='UTC')
...
def run_test(noise_thresh=None):
    tm=TradeManager(initial_sl=1.0,hard_tp=2.5,breakeven_trigger=0.50,trail_trigger=2.0,trail_dist=0.75,trail_dist_s=0.50,regime_tighten=0.40,max_hold=18,mae_guard_retrace=2.5)
    ...
    for i in range(max(config.seq_len_m15,20),len(m15f)):
        ...
        if pos!=0 and tm.state is not None:
            ...
            # Noise filter check before SL/TP
            noise_exit=False
            if noise_thresh is not None and len(ab)<=2:
                cur_r=(price-s2.entry_price)/sd2 if pos==1 else(s2.entry_price-price)/sd2
                if cur_r<noise_thresh:
                    epx=price;er=f'noise_{cur_r:.2f}R';noise_exit=True;noise_exits+=1
            if not noise_exit:
        ...
        listen=False
        lots=tm.compute_position_size(bal,h1_atr,price,config.risk_pct,1.0)
    ...
    return n,wr,pf,pnl,noise_exits,round(np.mean([t['pnl_r']for t in wins])if wins else 0,3),round(np.mean([t['pnl_r']for t in losses])if losses else 0,3)

print('BAR-1 NOISE FILTER — May 2026 (20 days)')
print(f"{'Thresh':>8s} {'Trades':>7s} {'WR':>6s} {'PF':>6s} {'PnL':>9s} {'NoiseEx':>8s} {'AvgW':>7s} {'AvgL':>7s}")
print('-'*65)
for th in [None,-0.50,-0.40,-0.30,-0.25,-0.20,-0.15,-0.10]:
    n,wr,pf,pnl,nex,aw,al=run_test(th)
    label=f'OFF' if th is None else f'{th:+.2f}R'
    print(f'{label:>8s} {n:7d} {wr:5.1f}% {pf:5.2f} ${pnl:>8,.0f} {nex:8d} {aw:+6.3f}R {al:+6.3f}R')
